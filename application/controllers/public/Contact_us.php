...
class Contact_us extends Public_Controller {
	...
	public function index() {
		...
		$this->vars['latitude'] = NULL !== __session('latitude') ? __session('latitude') : 0;
		$this->vars['longitude'] = NULL !== __session('longitude') ? __session('longitude') : 0;
		$this->vars['api_key'] = NULL !== __session('google_map_api_key') ? __session('google_map_api_key') : 0;
	...
	public function save() {
		if ($this->input->is_ajax_request()) {
			...
			if ($this->validation()) {
				...
				$this->db->set('comment_url', prep_url($this->input->post('comment_url', true)));
