		...
		if ($this->input->is_ajax_request()) {
			...
			if ($this->validation()) {
				...
				if (_isNaturalNumber( $id )) {
					...
					if ($this->vars['status'] == 'success' && filter_var((string) $fill_data['current_semester'], FILTER_VALIDATE_BOOLEAN)) {
						$this->m_academic_years->deactivate_academic_years($id, 'current_semester');
					}
					if ($this->vars['status'] == 'success' && filter_var((string) $fill_data['admission_semester'], FILTER_VALIDATE_BOOLEAN)) {
						$this->m_academic_years->deactivate_academic_years($id, 'admission_semester');
					}
				} else {
					if (filter_var((string) $fill_data['current_semester'], FILTER_VALIDATE_BOOLEAN)) {
						$this->m_academic_years->deactivate_academic_years(0, 'current_semester');
					}
					if (filter_var((string) $fill_data['admission_semester'], FILTER_VALIDATE_BOOLEAN)) {
