...
class Employees extends Admin_Controller {
	...
	public function create_account() {
		if ($this->input->is_ajax_request()) {
			$id = _toInteger($this->input->post('id', true));
			if (_isNaturalNumber( $id )) {
				$employee = $this->model->RowObject($this->pk, $id, $this->table);
				$this->db->set('user_name', $employee->nik);
				$this->db->set('user_password', password_hash($employee->nik, PASSWORD_BCRYPT));
				$this->db->set('user_full_name', $employee->full_name);
				$this->db->set('user_email', $employee->email);
