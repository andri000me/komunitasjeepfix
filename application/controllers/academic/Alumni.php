...
class Alumni extends Admin_Controller {
	...
	public function create_account() {
		if ($this->input->is_ajax_request()) {
			$id = _toInteger($this->input->post('id', true));
			if (_isNaturalNumber( $id )) {
				$alumni = $this->model->RowObject($this->pk, $id, $this->table);
				$this->db->set('user_name', $alumni->identity_number);
				$this->db->set('user_password', password_hash($alumni->identity_number, PASSWORD_BCRYPT));
				$this->db->set('user_full_name', $alumni->full_name);
				$this->db->set('user_email', $alumni->email);
