	...
	public function save() {
		if ($this->input->is_ajax_request()) {
			...
			foreach($rows as $row) {
				...
				$email_exists = $this->model->email_exists($exp[6]);
				if ( $identity_number_exists || $email_exists ) continue;
				...
				$this->db->set('email', trim($exp[6]));
				$this->db->set('street_address', trim($exp[7]));
