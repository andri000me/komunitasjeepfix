	...
	<?php if ( ! in_array($this->uri->segment(1), ['sambutan-kepala-sekolah', 'sambutan-rektor', 'sambutan-ketua', 'sambutan-direktur'])) { ?>
		<div class="card rounded-0 border border-secondary mb-3">
			...
			<div class="card-body">
				...
				<p class="card-text text-justify"><?=word_limiter(strip_tags(get_welcome()), 20);?></p>
			</div>
			<div class="card-footer text-center">
				<small class="text-muted text-uppercase"><a href="<?=site_url(forewords_route());?>">Selengkapnya</a></small>
			</div>
		</div>
	<?php } ?>
	...
	<form class="card p-1 border border-secondary mt-2 mb-2 rounded-0">
		<div class="input-group">
			...
			<div class="input-group-append">
				<button type="button" onclick="if (event.keyCode == 13) { subscribe(); return false; }" class="btn subscribe-button rounded-0"><i class="fa fa-envelope"></i></button>
			</div>
		</div>
	</form>
