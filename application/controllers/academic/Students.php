...
class Students extends Admin_Controller {
	...
	public function profile() {
		...
		if (_isNaturalNumber( $id )) {
			...
			$this->vars['academic'] = $this->vars['academic_references'] = $this->vars['students'] = true;
			$this->vars['content'] = 'academic/student_profile_preview';
			$this->load->view('backend/index', $this->vars);
		} else {
			show_404();
		}
	...
	public function create_account() {
		if ($this->input->is_ajax_request()) {
			$id = _toInteger($this->input->post('id', true));
			if (_isNaturalNumber( $id )) {
				$student = $this->model->RowObject($this->pk, $id, $this->table);
				$username_exists = $this->model->username_exists($student->identity_number);
				if ( $username_exists ) {
					$this->vars['status'] = 'error';
					$this->vars['message'] = 'Nama pengguna sudah digunakan!';
				} else {
					$this->db->set('user_name', $student->identity_number);
					$this->db->set('user_password', password_hash($student->identity_number, PASSWORD_BCRYPT));
					$this->db->set('user_full_name', $student->full_name);
					$this->db->set('user_email', $student->email);
					...
					$query = $this->db->get_compiled_insert('users');
	...
	public function create_accounts() {
		if ($this->input->is_ajax_request()) {
			...
			foreach($query->result() as $row) {
				$username_exists = $this->model->username_exists($row->identity_number);
				if ( $username_exists ) continue;
