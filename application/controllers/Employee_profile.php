...
class Employee_profile extends Admin_Controller {
	...
	public function index() {
		...
		$this->vars['query'] = $this->model->RowObject('id', $id, 'employees');
		...
		if ($this->input->is_ajax_request()) {
			...
			if (_isNaturalNumber( $id )) {
				if ($this->validation( $id )) {
					...
					$this->vars['status'] = $this->model->update($id, 'employees', $fill_data) ? 'success' : 'error';
	...
	public function email_exists( $email ) {
		...
		$user_profile_id = __session('user_profile_id');
		$this->load->model(['m_students', 'm_employees', 'm_users']);
		$student_exists = $this->m_students->email_exists( $email );
		$employee_exists = $this->m_employees->email_exists( $email, $user_profile_id );
